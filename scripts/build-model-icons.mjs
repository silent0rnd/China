...
const ICONS = {
  ...
  // Трал для негабарита: низкая грузовая площадка между тележками.
  'lowbed-platform': () =>
    contactShadow(0, 0, 30, 12) +
    box(-4, -1.5, 0, 1.3, 3, 0.55) + // задняя тележка
    box(-2.9, -1.3, 0.55, 1.2, 2.6, 0.75) + // подъём к площадке
    box(-1.9, -1.5, 0.4, 4.2, 3, 0.35, tint('#4a555f', '#2d363f', '#1b2229')) + // низкая площадка
    box(2.4, -1.3, 0.55, 1.1, 2.6, 0.75) + // подъём к седлу
    box(3.4, -1.4, 0, 1.2, 2.8, 0.55) + // передняя тележка
    box(3.3, -1.2, 0.55, 1.3, 2.4, 1.6) + // тягач
    lamp(4.6, -0.8, 1, GLOW, 2) +
    lamp(4.6, 0.8, 1, GLOW, 2) +
    lamp(-4, -0.9, 0.7, SIGNAL, 1.8),

  // Фигурка для масштаба: рядом с ней габарит груза читается физически.
  // Высота выдержана в тех же модельных единицах, что и контейнер
  // (2.8 единицы = 2.6 м), поэтому 1.95 единицы дают правдоподобный рост.
  figure: () =>
    contactShadow(0, 0, 9, 4) +
    box(-0.26, -0.24, 0, 0.22, 0.48, 0.85) + // левая нога
    box(0.04, -0.24, 0, 0.22, 0.48, 0.85) + // правая нога
    box(-0.34, -0.34, 0.85, 0.68, 0.68, 0.75, tint('#8a4744', '#66302e', '#42201e')) + // корпус
    box(-0.22, -0.22, 1.6, 0.44, 0.44, 0.35), // голова
